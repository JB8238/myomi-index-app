...
- 同じフィルタ条件で、馬連（UMAREN）を以下の買い方で集計
  ...
  - umaren_axis1 : 抽出馬内の指数1位（score順位1位）を軸に流す
...
def ensure_numeric_from_text(df: pd.DataFrame, cols: List[str]) -> None:
    """
    文字列/全角混在の可能性がある列を、半角化→数値化。
    """
    for c in cols:
        if c in df.columns:
            df[c] = pd.to_numeric(df[c].apply(to_halfwidth_digits), errors="coerce")
...
def umaren_axis1_bets_hit(selected: pd.DataFrame, actual_pair: Optional[Tuple[int, int]]) -> Tuple[int, int]:
    """
    軸1頭流し：selected 内の score順位1位を軸に、残りへ流す。
    """
    if len(selected) < 2:
        return 0, 0
    ...
    others = set(int(x) for x in sel.iloc[1:]["馬番_num"].tolist())
    bets = len(others)
    if bets <= 0:
        return 0, 0
...
def build_race_meta(df: pd.DataFrame) -> Dict[str, Dict[str, object]]:
    """
    race_id ごとに、実際の1-2着馬番ペアと馬連配当を事前計算。
    """
    ...
    for rid, r in df.groupby("race_id"):
        ...
        meta[rid] = {"actual_pair": actual_pair, "umaren_payout": float(umaren)}
...
def calc_umaren_for_config(picked: pd.DataFrame, race_meta: Dict[str, Dict[str, object]], mode: str) -> Tuple[int, float, int]:
    ...
    for rid, sel in picked.groupby("race_id"):
        ...
        if mode == "umaren_box":
            ...
            bets, hit = umaren_axis1_bets_hit(sel, actual_pair)
        else:
...
def main() -> None:
    ...
    # コンソール表示
    show_cols = [
        "rlevel", "topk", "place_low_min", "place_high_max", "pop_min", "pop_max", "diff_pop_min",
