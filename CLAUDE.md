...
```bash
cd ../妙味度指数_jvlink
.venv\Scripts\python.exe fetch_race_data.py --sid UNKNOWN --from <YYYYMMDD>000000 --option 4
.venv\Scripts\python.exe fetch_race_data.py --sid UNKNOWN --dataspec SNPN --from <YYYYMMDD>000000 --option 4
.venv\Scripts\python.exe fetch_weather.py --sid UNKNOWN --date <YYYYMMDD>
# Output: 妙味度指数_jvlink/data_out/{ra,se,ck,we,jc}.csv (this repo's preprocessing.py reads these)
```
Re-run `fetch_weather.py` any time you need the latest 馬場状態/騎手変更 mid-day (e.g. after a jockey change is announced) — but re-run `fetch_race_data.py --option 4` (RA/SE) first too, since some downstream logic (レースレベル timing checks, etc.) still depends on RA/SE being reasonably fresh even though WE/JC themselves come from a separate live channel.

**Always use `--option 4` for RA/SE, including the very first call of the day** — not just as an exhaustion fallback. Two independent reasons, confirmed 2026-08-26: (1) `option=1`/`2` are incremental — re-running with a date range already consumed this session returns `JVOpen failed with code -1`. (2) More subtly, even a same-day `--option 2` re-run that does *not* error can silently fail to refresh what you actually need: JV-Data's "今週データ" incremental feed does not reliably resend a race's record just because its confirmation status changed (未確定"0" → confirmed 馬場状態コード) if that race was already delivered once earlier in the day — this reproduced exactly as "馬場状態 stays blank for already-finished races even after re-running `fetch_race_data.py --option 2` then `preprocessing.py`" in real same-day use. `option=4`（セットアップデータ）always returns the full current state regardless of prior incremental consumption, so it's immune to both problems. `fetch_race_data.py`'s writers for `ra.csv`/`se.csv`/`ck.csv` use upsert-merge semantics (existing rows are kept and only matching keys get overwritten by the fresh pull, never a blind full-file replace — see "JV-Link Integration" below), so re-running `--option 4` repeatedly through the day, even with a narrow same-day `--from`, is safe and never destroys previously-fetched history.
...
### WE (天候馬場状態) / JC (騎手変更) — live meeting-day changes before RA/SE are refreshed

RA's 芝馬場状態コード/ダート馬場状態コード are **only populated once a race is confirmed** — for a same-day, not-yet-run race (データ区分="2", 出馬表 stage) both are always `"0"`, even minutes before post time. This is not a bug in `fetch_race_data.py`; JV-Data genuinely withholds this field until results are in. Likewise, SE's `騎手名略称` only reflects a jockey substitution once `se.csv` is re-fetched — until then it silently keeps showing the pre-change jockey (confirmed 2026-08-09: 新潟/中京 races with a TARGET-confirmed 前日 jockey change still showed the old jockey in `se.csv` after a fresh `--option 4` pull, because `se.csv` itself just isn't the right channel for this — see below). Getting *both* live, before RA/SE are re-fetched, needs an entirely different call:
...
- **0B14 is not weather-only** — per JV-Data4901.pdf's データ種別一覧, 速報開催情報(一括) bundles 天候馬場状態(WE)・出走取消/競走除外(AV)・**騎手変更(JC)**・発走時刻変更(TC)・コース変更(CC) in the same feed. `fetch_weather.py` (name kept for continuity, despite now covering more than weather) parses both `WE` and `JC` record types out of the same `read_all()` stream and writes `data_out/we.csv` and `data_out/jc.csv`. AV/TC/CC are not parsed yet (no reported need so far) — same mechanism would apply if they're needed later.
- Per the spec: "速報開催情報は提供後に稀に取り消す場合があります…取得の度に前回取得の情報を破棄し、データ取得時点で含まれている情報のみを利用してください" — a change can be announced then retracted. `fetch_weather.py` already fully overwrites `we.csv`/`jc.csv` on every run rather than merging with prior output, so retractions are handled correctly as long as it's re-run.
...
- **Mid-day re-run gotcha (found 2026-08-02) and fix**: `ra.csv`/`se.csv` are static snapshots on disk — they only reflect races confirmed as of the *last* `fetch_race_data.py` run, not the current moment. If you re-run `fetch_weather.py` + `preprocessing.py` mid-day without first re-running `fetch_race_data.py` to refresh RA, any race that has since finished still shows RA's 馬場状態コード as `"0"` (stale), so `_classify_baba_jotai` fell back to WE's *current* venue-wide reading for it too — silently overwriting an already-run race's 馬場状態 with whatever the track condition happens to be *now*, even if it changed since that race ran. Fixed by comparing each race's RA `発走時刻` (start time) against the current wall-clock time (`_post_time_passed`): if a race's post time has already passed but RA is still `"0"`, `preprocessing.py` now leaves 馬場状態 empty for that race (does **not** fall back to WE) and prints a warning listing the affected races, instead of silently using a value that may be wrong. The correct operational order for a mid-day re-run is still: refresh RA/SE *before* running `fetch_weather.py`/`preprocessing.py`, so already-finished races pick up their own confirmed code — this fix is a safety net for when that step is forgotten, not a replacement for it.
  - **Update 2026-08-26**: refreshing RA/SE with `--option 2` (as this note originally said) turned out to be insufficient on its own — real same-day use showed 馬場状態 staying blank for already-finished races even *after* re-running `fetch_race_data.py --option 2` then `preprocessing.py`, i.e. the safety net above kept firing despite doing the "correct" re-run. Root cause: JV-Data's `option=2`（今週データ）incremental feed does not reliably resend a race's record just because its confirmation status changed, if that race was already delivered once earlier the same day — no error, just silently stale. See the `--option 4` guidance in Step 0 above, which is now the standard recommendation for RA/SE (not just an exhaustion-error fallback) precisely because of this.
- **JC (騎手変更) — found 2026-08-09, fixed same day**: unlike 馬場状態, a jockey substitution is *not* a "value that changes over the day and needs a staleness check" — a JC record is a point-in-time, race+馬番-scoped announcement of "changed from X to Y", so once captured it applies regardless of whether the race has since started. `record_parsers.py`'s `JC_SPEC` (161 bytes, flat — no repeat groups) parses 変更後/変更前 of 負担重量・騎手コード・騎手名・騎手見習コード. `fetch_weather.py` dedupes by (レース番号+馬番), keeping only the record with the latest `発表月日時分` per horse (a horse can get re-announced more than once; last one wins). `preprocessing.py` merges `jc.csv` on `RACE_KEY + 馬番` right after the `騎手名略称→騎手名` rename, and overrides 騎手名 with JC's value via `combine_first` wherever a JC row exists — no `allow_we_fallback`-style gate needed, since (unlike WE) there's no risk of a JC record legitimately being "wrong for an already-run race": the announcement is inherently tied to that specific race. Confirmed against a real case: 新潟1R 馬番13 (今村聖奈→石神深道, announced 前日) — `se.csv`'s `騎手名略称` still showed 今村聖奈 even after a fresh `--option 4` RA/SE pull (SE genuinely isn't the right channel for this, not a staleness issue fixable by re-fetching RA/SE more aggressively), while `jc.csv` from the same `--dataspec 0B14` call `fetch_weather.py` already made for WE correctly carried the change.

### CK (出走別着度数) — running-style tendency and course/distance aptitude

`record_parsers.py`'s `CK_SPEC` covers only the horse-level portion (項番1-88, bytes 1-1384) of the 6870-byte CK record — the jockey/trainer/owner/breeder blocks that follow are out of scope. CK comes from a **different dataspec than RA/SE**: use `--dataspec SNPN` (not `SNAP`, which was retired 2023-08-08 for a producer-code-width change — `SNAP` silently stops returning data after 2023-07-31). `fetch_race_data.py --dataspec SNPN --from 20230808000000 --option 4` → `data_out/ck.csv`. `fetch_race_data.py` writes every output CSV in `BATCH_SIZE`-row (default 20000) streaming batches rather than buffering the whole run in memory, so a single wide-range pull (e.g. the full 2023-08-08–present CK history in one call) no longer needs to be chunked into ~6-month windows — the earlier `MemoryError` in 32bit Python (`flatten_wide` blowing up mid-pull on a multi-year CK request) was a buffering issue in this script, not a JVOpen/JVRead limitation, and is fixed by the streaming rewrite. (436 columns: cumulative 着回数 by track/条件/distance-bracket/venue×surface, plus 脚質傾向 = JRA-VAN's own precomputed 逃げ/先行/差し/追込 counts). Unlike `horse_race_history.csv`, CK rows are already cumulative "as of entry time" snapshots — no need to walk history yourself, just fetch the target date's CK the same way as RA/SE (option=2 for 今週データ works, confirmed against real not-yet-run races).
...
**欠損＝未経験の扱い（`MISSING_AS_CATEGORY_FEATURES`）**: 脚質傾向・コース複勝率・距離帯複勝率はNaNになる理由が「そのコース/距離帯/そもそも初出走で過去実績が無い」という意味のある状態であり、データ不備ではない（`core/ck_features.py`参照）。この3特徴量に限り、`core/strategy_engine.py`の`_label_feature`が欠損値を専用ラベル（`MISSING_LABEL = "欠損"`）に変換してからビン留め・マイニングする（欠損行を捨てず、他のビンと同格の1カテゴリとして検証対象にする）。`judge()`側の`_match_one`もこの3特徴量に限り欠損値を確定的に判定する（ルールのラベルが「欠損」ビンなら一致、それ以外の実値ビンなら不一致——どちらにせよ✅/空になり、△（判定保留）にはならない）。人気乖離・利益度上昇値など他の特徴量の欠損は、外部データ（smartrc等）の取得漏れやトラッキング開始前のためで意味が異なるため対象外のまま（欠損時は従来通り△に倒す）。この区別は「意味のある欠損」と「データパイプラインの穴」を混同しないための意図的な線引き。

`人気乖離`（推定人気ベース）は上記の理由でこの「欠損＝カテゴリ化」の対象外だが、その欠損率自体は無視できない別問題として残っている——`推定人気`はsmartrc.jpのスクレイピングに依存しており、この機能が導入される前の日付（全199日中85日、2026-08-02時点)は`推定人気`が全頭欠損のままで、`人気乖離`を参照するルールは判定不能(△)になる。ライブ運用日（Step 1後に`scripts/merge_smartrc_to_preprocessed.py`を実行済みの当日）はこの欠損は本来ゼロになるはずなので、本番での△率は過去データ全体で計測した数値より低くなるのが正常。
